...
void test_handler1()
{
    printf("Matched GET age\n");
}
void test_handler2()
{
    printf("Matched GET home\n");
}
void test_handler3()
{
    printf("Matched POST profile\n");
}
void test_handler4()
{
    printf("Matched PUT user\n");
}
void test_handler5()
{
    printf("Matched GET post\n");
}
...
int main()
{
    ...
    // 测试重复注册（现在支持重复注册，返回应该总是 true）
    if (router.addRoute("GET /hello/<name>/age", (void*)test_handler1))
    {
        printf("Registered another handler for the same route: GET /hello/<name>/age\n");
    }
    ...
    if (router.match("GET /user/99/post/1024", res))
    {
        printf("Request: GET /user/99/post/1024 -> uid=%s, pid=%s, ",
               res.params["uid"].c_str(),
               res.params["pid"].c_str());
        if (!res.handlers.empty())
            ((void (*)())res.handlers[0].handler)();
    }
    ...
    {
        printf("Request: GET /a/val1/c/val2/e -> b=%s, d=%s, ", res.params["b"].c_str(), res.params["d"].c_str());
        if (!res.handlers.empty())
            ((void (*)())res.handlers[0].handler)();
    }
